        ...
        if len(images) != 0:           
            ...
            with torch.no_grad():
                ...
                if not isinstance(images[0], torch.Tensor):
                    images = [transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])(img) for img in images]
...
class DetectionModel:
    ...
    def getCropImages(self, images):  
        ...
        for i, image in enumerate(images): 
            ...
            crop = cropImage(image, i, box)
            imagesCrop.append(crop)
        return imagesCrop
        
        
elbows = sorted(glob(os.path.join('elbow', "*.png")))    
crops = sorted(glob(os.path.join('crop', "*.png")))    

modelDetection = DetectionModel.load('models/frcnn_elbow.pth') 
modelCrop512 = DensenetModel.load('models/crop2-fix-512-7_acc.pth', 'Crop512')
modelRaid512 = DensenetModel.load('models/raid5-512-8_acc.pth', 'Raid512')
modelRaid1024 = DensenetModel.load('models/raid5-fix-1024-12_loss.pth', 'Raid1024')

with open('auc_results.csv', mode='w') as csvfile:
    writer = csv.writer(csvfile, delimiter=',')
    writer.writerow(["Image", "Links", "Results1", "Results2", "Results3"])
    for i, elbow in enumerate(elbows):
        elbowImage = Image.open(elbow)
        cropImage = Image.open(crops[i])
        predCrop512 = modelCrop512.predict_for_study([cropImage])
        predRaid512 = modelRaid512.predict_for_study([elbowImage])
        predRaid1024 = modelRaid1024.predict_for_study([elbowImage])
        writer.writerow([elbow, predCrop512, predRaid512, predRaid1024])
